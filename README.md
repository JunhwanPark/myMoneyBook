...
앱이 정상적으로 데이터를 읽고 쓰기 위해서는 구글 스프레드시트의 첫 번째 줄(Header)이 아래 구조와 유사하게 세팅되어야 합니다. (※ 본인의 `backend/Code.js` 로직에 맞춰 시트 이름이나 열 순서는 유연하게 조정 가능합니다.)

#### 1. 내역 시트 (예: `Records` 또는 `내역`)

사용자의 가계부 수입/지출 내역이 쌓이는 메인 시트입니다.

| A (ID)    | B (Date)     | C (Type)                | D (Category) | E (Amount) | F (Memo)                             |
| :-------- | :----------- | :---------------------- | :----------- | :--------- | :----------------------------------- |
| `고유 ID` | `2026-03-15` | `expense` (또는 income) | `cat_food`   | `15000`    | `항목명\|결제수단\|할인액\|상세메모` |

- **Memo 규격:** 프론트엔드는 파이프(`|`) 기호를 기준으로 항목명, 결제수단(현금/카드명), 할인액, 상세 메모를 구분하여 파싱합니다.

#### 2. 설정 시트 (예: `Settings` 또는 `카테고리`)

카테고리 목록과 신용카드 목록을 관리하는 시트입니다.

| A (Type)  | B (Value)    | C (Label)                           |
| :-------- | :----------- | :---------------------------------- |
| `expense` | `cat_food`   | `식비`                              |
| `income`  | `inc_salary` | `급여`                              |
| `card_KR` | `card_1`     | `신한카드\|14` (카드명\|결제기준일) |
| `card_CN` | `card_2`     | `招商银行\|말일`                    |

- **Type 분류:** `expense`(지출 카테고리), `income`(수입 카테고리), `card_KR`(한국 카드), `card_CN`(중국 카드) 등으로 구분합니다.
- **카드 Label 규격:** 카드명과 결제 기준일(예: 14일, 말일)을 파이프(`|`) 기호로 구분하여 입력합니다.
